...
parser.add_argument('-tdp', '--test_data_path', type=str, default='./data_f3/validation')
...
if __name__ == "__main__":    
    ...
    for batch_data in tqdm(test_dataloader):
        ...
        out_dice[img_name] = max(dice_list)
        out_iou[img_name] = per_iou  # Store IoU values

        cur_dice_dict = OrderedDict()
        cur_iou_dict = OrderedDict()

        for i, (dice, iou) in enumerate(zip(dice_list, iou_list)):
            cur_dice_dict[f'{i}'] = dice
            cur_iou_dict[f'{i}'] = iou

        out_dice_all[img_name] = cur_dice_dict
        out_iou_all[img_name] = cur_iou_dict
    ...
    # Save IoU scores
    iou_save_name = args.save_name.replace('.py', '_iou.py')
    with open(iou_save_name, 'w') as f:
        f.writelines(f'# mean IoU: \t{np.mean(all_iou_list)}\n')
        f.writelines('iou_Ts = {')
        for k, v in out_iou.items():
            f.writelines(f'\'{str(k[0])}\': {v},\n')
        f.writelines('}')

    # Store final IoU dictionary
    final_iou_dict = OrderedDict()
    for k, v in out_dice_all.items():
        organ = k.split('/')[-4]
        final_iou_dict[organ] = OrderedDict()
    for k, v in out_iou_all.items():
        organ = k.split('/')[-4]
        final_iou_dict[organ][k] = v
        
    with open(args.save_name.replace('.py', '.json'), 'w') as f:
        json.dump(final_dice_dict, f, indent=4)

    #Save IoU scores in JSON
    iou_json_name = args.save_name.replace('.py', '_iou.json')
    with open(iou_json_name, 'w') as f:
        json.dump(final_iou_dict, f, indent=4)
