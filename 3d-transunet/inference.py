...
parser.add_argument('--n_class', default=3, type=int, help="")
...
# decoder_only
args.config='ath to config file'
...
def _get_gaussian(patch_size, sigma_scale=1. / 8) -> np.ndarray:
    ...
    # print('gaussian_importance_map shape: ',gaussian_importance_map.shape)
    return gaussian_importance_map


gaussian_mask = torch.from_numpy(_get_gaussian(patch_size)[np.newaxis, np.newaxis]).cuda().half().clamp_min_(1e-4)
# gaussian_mask = gaussian_mask.repeat(1, 20, 1, 1, 1)
        ...
        for y in steps[1]:
            ...
            for z in steps[2]:
                ...
                with torch.no_grad():
                    ...
                    seg_pro=seg_pro[0]
                    # print("Start")
                    # print('seg_pro: ',seg_pro[0])
                    # print('seg_pro type: ',len(seg_pro))
                    # print('isinstance(seg_pro, dict): ',isinstance(seg_pro, dict))
                    # print("'is_max_hungarian' in model_params.keys(): ",'is_max_hungarian' in model_params.keys())
                    # print("model_params['is_max_hungarian']: ",model_params['is_max_hungarian'])
                    if args.num_ds is not None and isinstance(seg_pro, dict) and ('is_max_hungarian' in model_params.keys() and model_params['is_max_hungarian']):
                        ...
                        print('Class 4')
                        # print('Original seg_pro[0]: ',seg_pro[0])
                        # print('Original seg_pro[1]: ',seg_pro[1])
                        seg_pro = seg_pro[0]
                        # print('****** seg_pro type: ',type(seg_pro))
                        print('****** seg_pro keys: ',seg_pro.keys())
                        # print('seg_pro: ',seg_pro)
                        seg_pro = seg_pro['pred_masks']
                        # print('****** seg_pro[pred_logits] shape : ',seg_pro['pred_logits'].shape)
                        # print('****** seg_pro[pred_masks] shape : ',seg_pro['pred_masks'].shape)
                        # print('****** seg_pro[aux_outputs] len: ',len(seg_pro['aux_outputs']))
                        # print('****** gaussian_mask: ',gaussian_mask.shape)
                        # print('****** seg_pro[aux_outputs] shape: ',seg_pro['aux_outputs'].shape)


                        if torch.sum(seg_pro[0,:,0,0,0]) != 1: 
                            seg_pro = torch.softmax(seg_pro, dim=1)
                        print('seg_pro shape: ',seg_pro.shape)
                        print('gaussian_mask: ',gaussian_mask.shape)
                        _pred = seg_pro * gaussian_mask
                        print('_pred shape: ',_pred.shape)
                        print()
                        temp=prob_map[:, :3, lb_x:ub_x, lb_y:ub_y, lb_z:ub_z]
                        # prob_map=temp  
                        print('temp shape: ',temp.shape)
                        print('Other var: ',lb_x,ub_x,lb_y,ub_y,lb_z,ub_z)
                        prob_map[:, :, lb_x:ub_x, lb_y:ub_y, lb_z:ub_z] += _pred
                    else:
                        print('Class 5')
                        if args.is_sigmoid:
