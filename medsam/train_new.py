...
parser.add_argument(
    "-data_root", type=str, default="path to data dir",
...
parser.add_argument(
    "-resume", type=str, default='work_dir/medsam_lite_latest_500.pth',
    help="Path to the checkpoint to continue training."
)
parser.add_argument(
    "-work_dir", type=str, default="path to out dir",
...
def pad_bounding_boxes(bboxes, max_bboxes=MAX_BBOXES):
    padded_bboxes = np.zeros((max_bboxes, 4), dtype=np.float32)
    num_bboxes = min(len(bboxes), max_bboxes)
...
class NpyDataset(Dataset): 
    ...
    def create_data_points(self):
        for index in range(len(self.gt_path_files)):
            ...
            contours, _ = cv2.findContours(gt2D, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            bounding_boxes = []
...
for epoch in range(start_epoch + 1, num_epochs):
    ...
    epoch_dice_loss = [1e10 for _ in range(len(train_loader))]
    epoch_iou_gt = [0 for _ in range(len(train_loader))]
    m_iou = 0 
    ...
    for step, batch in enumerate(pbar):
        ...
        m_iou += iou_gt.mean()*iou_gt.shape[0]
        l_iou = iou_loss(iou_pred, iou_gt)
        #loss = mask_loss + l_iou
        loss = mask_loss + iou_loss_weight * l_iou
        epoch_dice_loss[step] = l_seg.item()
        epoch_loss[step] = loss.item()
        loss.backward()
        optimizer.step()
        optimizer.zero_grad()
        pbar.set_description(f"Epoch {epoch} at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}, loss: {loss.item():.4f}, batch_dice_loss: {l_seg.mean():4f} ,batch_iou: {iou_gt.mean():4f}")
        epoch_iou_gt[step] = iou_gt.mean()
    m_iou = m_iou/len(train_loader)
    epoch_end_time = time()
    epoch_loss_reduced = sum(epoch_loss) / len(epoch_loss)
    dice_loss_reduced = sum(epoch_dice_loss) / len(epoch_dice_loss)
    ...
    torch.save(checkpoint, join(work_dir, "medsam_lite_latest_500.pth"))
    if epoch_loss_reduced < best_loss:
        print(f"New best loss: {best_loss:.4f} -> {epoch_loss_reduced:.4f}, dice loss: {dice_loss_reduced:4f}")
        best_loss = epoch_loss_reduced
        checkpoint["best_loss"] = best_loss
        torch.save(checkpoint, join(work_dir, "medsam_lite_best_500.pth"))
        torch.save(model_weights, os.path.join(work_dir, f"model_best.pth"))
    if epoch%10==0 or epoch==num_epochs-1:
        torch.save(model_weights, os.path.join(work_dir, f"model_{epoch}.pth"))
