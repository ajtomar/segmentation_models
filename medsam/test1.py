...
parser.add_argument(
    ...
    type=str,
    default="workdir/medsam_lite_best_500.pth",
...
# medsam_lite_checkpoint_path = '/home/ajay/scratch/models/MedSAM/workdir_f0_100epohs/model_20.pth'
...
def calculate_iou(gt_mask, pred_mask, cls=1):
    ...
    return iou_score
# def calculate_iou(result, reference):
...
def MedSAM_infer_npz(gt_path_file):
    ...
    # log_file = open("output_log_f2_20epochs.txt", "a")
    # sys.stdout = log_file
    ...
    if (not isfile(join(pred_save_dir, task_folder, npz_name))) or overwrite:
        ...
        for i in range(img_3D.shape[0]):
            ...
            print('img_256_tensor shape: ',img_256_tensor.shape)
            with torch.no_grad():
                image_embedding = medsam_lite_model.image_encoder(img_256_tensor)

            gt = gt_3D[i,:,:] # (H, W)
            # dice_loss.append(seg_loss(torch.tensor(img_2d), torch.tensor(gt))) #2d losses
            ...
            for label_id in label_ids:
                ...
                if np.sum(gt2D_padded) > 0:
                    ...
                    iou = calculate_iou(gt,sam_mask)
                    ious.append(iou)
                    dice = compute_dice_coefficient(torch.tensor(gt)>0 , torch.tensor(sam_mask)>0)
                    dice_coeff.append(dice)
                    
                    print(np.unique(sam_mask), iou, dice)
                    seg_3D[i, sam_mask>0] = label_id
                    box_list[i][label_id] = box
        # print("frame wise dice_loss:", dice_loss)    
        # print("frame wise iou_loss:", ious)  
        # print("frame wise dice_coeff:", dice_coeff)  
        #print("main m_IOU: ",sum(ious)/len(ious), "m_DICE_COEFF: ", sum(dice_coeff)/len(dice_coeff))
        ...
        if save_overlay:
            for idx in range(seg_3D.shape[0]):
            # idx = int(seg_3D.shape[0] / 2)
                ...
                ax[0].set_title(f"Image m_IOU: {sum(ious)/len(ious)} , m_DICE_COEFF: {sum(dice_coeff)/len(dice_coeff)}")
                ...
                for label_id, box_256 in box_dict.items():
                    ...
                    # Normalize the RGB values to be within the 0-1 range
                    color_gt = [c / 255.0 for c in color_gt]
                    color_pred = [c / 255.0 for c in color_pred]
                    box_viz = resize_box(box_256, (newh, neww), (H, W))
                    show_mask(gt_3D[idx], ax[1], mask_color=color_gt)
                    show_box(box_viz, ax[1], edgecolor=color_gt)
                    show_mask(seg_3D[idx], ax[2], mask_color=color_pred)
                    show_box(box_viz, ax[2], edgecolor=color_pred)
                plt.tight_layout()
                # 
                
                # iou = calculate_iou(gt_3D[idx]>0,sam_mask[idx]>0)
                # dice = compute_dice_coefficient(torch.tensor(gt_3D[idx])>0 , torch.tensor(sam_mask[idx])>0).item()
                # print(gt_3D[idx].shape, sam_mask[idx].shape,  dice, iou)
                # print("Image id", npz_name.split(".")[0]+ "_"+str(idx)+ " IOU: ",iou, "m_DICE_COEFF: ", dice)

                print(join(png_save_dir, npz_name.split(".")[0] +"_"+str(idx)+ '.png'))
                plt.savefig(join(png_save_dir, npz_name.split(".")[0] +"_"+str(idx)+ '.png'), dpi=300)
                plt.close()	
        # print(npz_name)
        print(f"Volume Name: {npz_name}, IOU: {sum(ious)/len(ious)}, DICE: {sum(dice_coeff)/len(dice_coeff)}")
        final_iou.append(sum(ious)/len(ious))
        final_dice.append(sum(dice_coeff)/len(dice_coeff))
    ...
    with mp.Pool(processes=num_workers) as pool:
        with tqdm(total=len(gt_path_files)) as pbar:
            for i, _ in tqdm(enumerate(pool.imap_unordered(MedSAM_infer_npz, gt_path_files))):
                pbar.update()
                # break
    # print('Total length', len(final_dice))
    # print(f"Mean IOU: {sum(final_iou)/len(final_iou)}, Mean DICE: {sum(final_dice)/len(final_dice)}")
